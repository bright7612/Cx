...
class CxdpController extends AdminController{
...
//        var_dump($issuemenus[0]['issuemenus'][0]);exit;
        ...
        $builder->button('新增', array('class' => 'btn btn-success','href' => U('admin/Cxdp/addcontent?type='.$type)));
        $builder->button('已发布列表', array('class' => 'btn btn-info','href' => U('admin/Cxdp/content?title=1&type='.$type)))
            ->button('编辑中列表', array('class' => 'btn btn-warning','href' => U('admin/Cxdp/content?title=0&type='.$type)))
            ->button('已删除列表', array('class' => 'btn btn-danger','href' => U('admin/Cxdp/content?title=-1&type='.$type)))

            ->keyId()
            ->keyText('link','标题')
//            ->keyStatus()->keyText('sort','排序')
            ->keyText('issue_name','分类')->keyCreateTime()
            ->keyDoActionEdit("admin/Cxdp/addcontent/type/0/type/".$type.'?id=###')
            ->data($list)
...
//                    ->keySelect('video_type', '选择视频类别', '', $select)
    ...
    public function setWildcardStatus(){
        ...
        $builder = new AdminListBuilder();
        $builder->doSetStatus('dp_Issue_content', $ids, $status);
    }
