...
class DqzjController extends AdminController{
    public function content($page = 1, $r = 10,$type='1',$title=1)
    {
        ...
        if(CONTROLLER_NAME =='Dqzj'){
            $mapcontent['status']=1;
            if($groupid == 5){
                $mapcontent['group_id'] = array('like','%5%');
            }elseif ($groupid == 10){
...
//        var_dump($issuemenus[0]['issuemenus'][0]);exit;
        ...
        $builder->button('新增', array('class' => 'btn btn-success','href' => U('admin/Dqzj/addcontent?type='.$type)));
        $builder->button('已发布列表', array('class' => 'btn btn-info','href' => U('admin/Dqzj/content?title=1&type='.$type)))
            ->button('编辑中列表', array('class' => 'btn btn-warning','href' => U('admin/Dqzj/content?title=0&type='.$type)))
            ->button('已删除列表', array('class' => 'btn btn-danger','href' => U('admin/Dqzj/content?title=-1&type='.$type)))

            ->keyId()
            ->keyText('link','标题')
//            ->keyStatus()->keyText('sort','排序')
            ->keyImage("cover_id", "图片",array('width'=>50,'height'=>30))
            ->keyText('issue_name','分类')->keyCreateTime()
            ->keyDoActionEdit("admin/Dqzj/addcontent/type/0/type/".$type.'?id=###')
            ->data($list)
...
//            dump($issuemenus[0]['issuemenus'][1]);exit;
            ...
            if($groupid == 5){
                $mapcontent2['group_id'] = array('like','%5%');
            }elseif ($groupid == 10){
...
//            $select = array('0'=>'请选择','1'=>'宣传片','2'=>'合庆专题之路','3'=>'合庆镇环境综合整治治理行动纪实');
            if($type == 201 || $type == 190 || $type == 191 || $type == 205 || $type == 203 || $type == 207 || $type == 216 || $type == 219 || $type == 207) {
...
//                    ->keySelect('video_type', '选择视频类别', '', $select)
    ...
    public function setWildcardStatus(){
        ...
        $builder = new AdminListBuilder();
        $builder->doSetStatus('dq_issue_content', $ids, $status);
    }
