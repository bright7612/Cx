 ...
 * 主要定义后台公共函数库
 */
    function exportExcel($expTitle="党员基本信息汇总表",$expCellName="数组",$expTableData="asdf"){

        $xlsTitle = iconv('utf-8', 'gb2312', $expTitle);//文件名称
        $fileName = $_SESSION['account'].date('党员数据导出_Ymd');//or $xlsTitle 文件名称可根据自己情况设定
        ...
        $objPHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('O')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('Q')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('S')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('T')->setWidth('20');//设置列宽
        $objPHPExcel->getActiveSheet()->getColumnDimension('U')->setWidth('20');//设置列宽
...
function excel2Time($date, $time = false) {
	if (function_exists('GregorianToJD')) {
		if (is_numeric($date)) { $jd = GregorianToJD(1, 1, 1970);
		...
		/*There was a bug if Converting date before 1-1-1970 (tstamp 0)*/$ofs = (70 * 365 + 17 + 2) * 86400;
		$date = date("Y-m-d", ($date * 86400) - $ofs) . ($time ? " 00:00:00" : '');
	}
	return $date;
}

function dzznameGetId($name) {
    header("Content-Type:text/html;charset=utf-8");

    $info1 = M("dzz")->where("LOCATE(namestr,'".$name."') >0")->find();
    if($info1['id']){
        return $info1['id'];
    }else{
        $info2 = M("dzz")->where("LOCATE(namestr2,'".$name."') >0")->find();
        if($info2['id']){
            return $info2['id'];
        }else{
            $info3 = M("dzz")->where("LOCATE(namestr3,'".$name."') >0")->find();
            if($info3['id']){
                return $info3['id'];
            }else{
                return 0;
 ...
 * @return bool
 */
function isDzbAdmin($uid){
    if($uid){
        if(M("auth_group_access")->where(array("uid"=>$uid,"group_id"=>4))->find()){
            return true;
        }
    }
    return false;
...
function zbadminGetDzz(){
    $user = session("user_auth");
   return adminUidGetDzz($user['uid']);
}
...
function getMerge(&$start,&$jump,$merge){
    $str = array();
    $value = array();
    $s = "";
    $count = 0;
    ...
    for ($x=0; $x<=count($merge); $x++) {
        $str = str_split($merge[$x]);
        $type = "1";//1代表字母,2代表数字
        $value = NULL;
        $count = 0;
        $s = "";
        for($i=0; $i<=count($str); $i++) {
            $isnumber = ctype_digit($str[$i]);
            if($type=="1"&&$isnumber&&$s!=""){
                $value[$count]=PHPExcel_Cell::columnIndexFromString($s);
                $count++;
                $type = "2";
                $s = $str[$i];
            }else if($type=="2" && !$isnumber){
                $value[$count]=(int)$s;
                $count++;
                $type = "1";
                $s = "";
        ...
        for($i = $value[1]; $i <= $value[3]; $i++){
            for($j = $value[0]; $j <= $value[2]; $j++){
                if($i==$value[1]&&$j==$value[0]){
                    $arr = array(($value[3]-$value[1])+1,($value[2]-$value[0])+1);
                    //var_dump($arr);
                    $start[($i-1).($j-1)] = $arr;
                }else{
                    $jump[($i-1).($j-1)] = -1;
